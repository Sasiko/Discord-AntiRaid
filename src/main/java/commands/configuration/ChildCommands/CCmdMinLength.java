...
public class CCmdMinLength extends ChildCommand {

  public CCmdMinLength(Command parentCommand, String invoke) {
  ...
  public void action(String[] args, String content, GuildMessageReceivedEvent event) throws InsufficientPermissionException {
    ...
    int old_value = (int) Config.getConfig("MIN_LENGTH");

    if (Config.setConfig("MIN_LENGTH", value)) {
      EmbedBuilder embedBuilder = new EmbedBuilder();
      embedBuilder.setColor(Color.green);
      embedBuilder.setTitle("Config has been updated");
      embedBuilder.setDescription(String.format("Config **MIN_LENGTH** has been set from `%d` to `%d`", old_value, value));
  ...
  public String description() {
    return
        String.format("How long does the message have to be to get filtered. Current value is `%d`.", (int) Config.getConfig("MIN_LENGTH"));
  }
